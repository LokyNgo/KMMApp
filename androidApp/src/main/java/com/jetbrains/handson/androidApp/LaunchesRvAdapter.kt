...
class LaunchesRvAdapter(val context: Context, var launches: List<RocketLaunch>) : RecyclerView.Adapter<LaunchesRvAdapter.LaunchViewHolder>() {

    lateinit var callback: Callback

    interface Callback {
        fun onClickCloseIcon(launch: RocketLaunch)
    }
    ...
    inner class LaunchViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        private val iconClose = itemView.findViewById<ImageView>(R.id.icClose)
        ...
        fun bindData(launch: RocketLaunch) {
            ...
            missionDetailsTextView.setOnClickListener {
                val url = launch.links.article
                val intent = Intent(Intent.ACTION_VIEW, Uri.parse(url))
                context.startActivity(intent)
            }
            iconClose.setOnClickListener {
                callback.onClickCloseIcon(launch)
            }
        }
    }
}